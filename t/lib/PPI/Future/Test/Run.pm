...
sub run_testdir {
	...
	foreach my $codefile ( @code ) {
		...
		my $document = PPI::Future::Document->new( $codefile );
		ok( $document, "$codename: Lexer->Document returns true" );
		ok( _INSTANCE($document, 'PPI::Future::Document'), "$codename: Object isa PPI::Future::Document" );
		...
		SKIP: {
			...
			my $Dumper = PPI::Future::Dumper->new( $document );
			ok( _INSTANCE($Dumper, 'PPI::Future::Dumper'), "$codename: Object isa PPI::Future::Dumper" );
...
sub increment_testdir {
	...
	foreach my $codefile ( @code ) {
		...
		foreach my $chars ( 1 .. length($buffer) ) {
			...
			my $document = eval {
				PPI::Future::Document->new( \$string );
			};
			is(
				$@ => '',
			...
			is(
				ref($document) => 'PPI::Future::Document',
