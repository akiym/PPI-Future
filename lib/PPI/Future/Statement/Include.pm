package PPI::Future::Statement::Include;

=pod

=head1 NAME

PPI::Future::Statement::Include - Statements that include other code
...
Despite its name, the C<PPI::Future::Statement::Include> class covers a number
...
C<PPI::Future::Statement::Include> has a number of methods in addition to the standard
L<PPI::Future::Statement>, L<PPI::Future::Node> and L<PPI::Future::Element> methods.

=cut

use strict;
use PPI::Future::Statement                 ();
use PPI::Future::Statement::Include::Perl6 ();
...
sub type {
	...
	$keyword->isa('PPI::Future::Token::Word') and $keyword->content;
}
...
sub module {
	...
	$module->isa('PPI::Future::Token::Word') and $module->content;
}
...
sub module_version {
	...
	if ( $argument and $argument->isa('PPI::Future::Token::Operator') ) {
		return undef;
	}

	my $version = $self->schild(2) or return undef;
	return undef unless $version->isa('PPI::Future::Token::Number');
...
For now, PPI::Future assumes that any "module name" that is only a set of
...
sub version {
	...
	$version->isa('PPI::Future::Token::Number') ? $version->content : '';
}
...
sub version_literal {
	my $self    = shift;
	my $version = $self->schild(1) or return undef;
	$version->isa('PPI::Future::Token::Number') ? $version->literal : '';
}
...
sub arguments {
	...
	if (
		$args[-1]->isa('PPI::Future::Token::Structure')
		and
		$args[-1]->content eq ';'
	) {
		pop @args;
	...
	if ( $args[0]->isa('PPI::Future::Token::Number') ) {
		my $after = $args[1] or return;
		$after->isa('PPI::Future::Token::Operator') or shift @args;
	}
