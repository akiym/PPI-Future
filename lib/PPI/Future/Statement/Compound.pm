package PPI::Future::Statement::Compound;

=pod

=head1 NAME

PPI::Future::Statement::Compound - Describes all compound statements
...
C<PPI::Future::Statement::Compound> objects are used to describe all current forms
...
C<PPI::Future::Statement::Compound> has a number of methods in addition to the
standard L<PPI::Future::Statement>, L<PPI::Future::Node> and L<PPI::Future::Element> methods.
...
#####################################################################
# PPI::Future::Statement::Compound analysis methods
...
sub type {
	...
	if ( $Element->isa('PPI::Future::Token::Label') ) {
	...
	if ( $content =~ /^for(?:each)?\z/ ) {
		...
		if ( $Element->isa('PPI::Future::Token') ) {
			...
			return 'foreach' if $Element->isa('PPI::Future::Token::Symbol');
			return 'foreach' if $Element->isa('PPI::Future::Token::QuoteLike::Words');
		}
		if ( $Element->isa('PPI::Future::Structure::List') ) {
			return 'foreach';
		}
		return 'for';
	}
	return $TYPES{$content} if $Element->isa('PPI::Future::Token::Word');
	return 'continue'       if $Element->isa('PPI::Future::Structure::Block');
...
sub _complete {
	...
	if ( $type eq 'if' ) {
		...
		$child->isa('PPI::Future::Structure') or return '';
