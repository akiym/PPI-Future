...
PPI::Future::Statement::Package - A package statement
...
Most L<PPI::Future::Statement> subclasses are assigned based on the value of the
first token or word found in the statement. When PPI::Future encounters a statement
starting with 'package', it converts it to a C<PPI::Future::Statement::Package>
object.
...
completed in PPI::Future.

However, if the immediate parent of the package statement is the
top level L<PPI::Future::Document> object, then it can be considered to define
...
C<PPI::Future::Statement::Package> has a number of methods in addition to the standard
...
sub namespace {
	...
	$namespace->isa('PPI::Future::Token::Word')
		? $namespace->content
		: '';
}
...
sub version {
	...
	$version->isa('PPI::Future::Token::Structure')
		? ''
		: $version->content;
}
...
Note that if the PPI::Future DOM tree B<does not> have a PPI::Future::Document object at
as the root element, this will return false. Likewise, it will also return
false if the root element is a L<PPI::Future::Document::Fragment>, as a fragment of
...
sub file_scoped {
	...
	and $Document->isa('PPI::Future::Document')
	and ! $Document->isa('PPI::Future::Document::Fragment');
}
