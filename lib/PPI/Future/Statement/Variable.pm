package PPI::Future::Statement::Variable;

=pod

=head1 NAME

PPI::Future::Statement::Variable - Variable declaration statements
...
=head1 INHERITANCE
  ...
  isa PPI::Future::Statement::Expression
...
The main intent of the C<PPI::Future::Statement::Variable> class is to describe
...
will migrate deeper down the tree to either L<PPI::Future::Statement> or
L<PPI::Future::Node> to recognise this fact, but for now it stays here.
...
sub type {
	...
	# Get the type
	(_INSTANCE($schild[0], 'PPI::Future::Token::Word') and $schild[0]->content =~ /^(my|local|our|state)$/)
		? $schild[0]->content
		: undef;
}
...
sub symbols {
	...
	if ( _INSTANCE($schild[1], 'PPI::Future::Token::Symbol') ) {
		return $schild[1];
	}

	# If it's a list, return as a list
	if ( _INSTANCE($schild[1], 'PPI::Future::Structure::List') ) {
