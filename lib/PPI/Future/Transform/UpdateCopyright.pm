package PPI::Future::Transform::UpdateCopyright;

=pod

=head1 NAME

PPI::Future::Transform::UpdateCopyright - Demonstration PPI::Future::Transform class

=head1 SYNOPSIS

  my $transform = PPI::Future::Transform::UpdateCopyright->new(
      name => 'Adam Kennedy'
  );
...
B<PPI::Future::Transform::UpdateCopyright> provides a demonstration of a typical
L<PPI::Future::Transform> class.
...
sub new {
	...
	unless ( defined _STRING($self->name) ) {
		PPI::Future::Exception->throw("Did not provide a valid name param");
	}
...
sub document {
	...
	my $elements = $document->find( sub {
		$_[1]->isa('PPI::Future::Token::Pod') or return '';
