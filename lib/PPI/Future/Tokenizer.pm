package PPI::Future::Tokenizer;

=pod

=head1 NAME

PPI::Future::Tokenizer - The Perl Document Tokenizer

=head1 SYNOPSIS

  # Create a tokenizer for a file, array or string
  $Tokenizer = PPI::Future::Tokenizer->new( 'filename.pl' );
  $Tokenizer = PPI::Future::Tokenizer->new( \@lines       );
  $Tokenizer = PPI::Future::Tokenizer->new( \$source      );
...
PPI::Future::Tokenizer is the class that provides Tokenizer objects for use in
breaking strings of Perl source code into Tokens.

By the time you are reading this, you probably need to know a little
about the difference between how perl parses Perl "code" and how PPI::Future
...
PPI::Future uses a completely different approach by abandoning the (impossible)
...
In this approach, the tokenizer C<PPI::Future::Tokenizer> is implemented separately
from the lexer L<PPI::Future::Lexer>.

The job of C<PPI::Future::Tokenizer> is to take pure source as a string and break it
up into a stream/set of tokens, and contains most of the "black magic" used
in PPI::Future. By comparison, the lexer implements a relatively straight forward
...
gets exposed to people using the C<PPI::Future::Tokenizer> itself.
...
# we don't have to go and load all of PPI::Future.
use strict;
...
use PPI::Future::Exception::ParserRejection ();
...
Returns a new C<PPI::Future::Tokenizer> object on success, or throws a
L<PPI::Future::Exception> exception on error.
...
sub new {
	...
	my $self = bless {
		...
		# Parse state
		token        => undef,
		class        => 'PPI::Future::Token::BOM',
		zone         => 'PPI::Future::Token::Whitespace',
	...
	if ( ! defined $_[1] ) {
		...
		PPI::Future::Exception->throw("No source provided to Tokenizer");

	} elsif ( ! ref $_[1] ) {
		my $source = PPI::Future::Util::_slurp($_[1]);
		if ( ref $source ) {
		...
		PPI::Future::Exception->throw(ref($_[1]) . " is not supported as a source provider");
	}
...
When using the PPI::Future::Tokenizer object as an iterator, the C<get_token>
...
Return a L<PPI::Future::Token> object on success, C<0> if the Tokenizer had
...
sub get_token {
	...
	my $rv = eval {
		...
		while ( $line_rv = $self->_process_next_line ) {
			...
			# The defined() prevents a ton of calls to PPI::Future::Util::TRUE
...
Returns a reference to an ARRAY of L<PPI::Future::Token> objects on success
...
sub all_tokens {
	...
	my $ok = eval {
		...
		unless ( $self->{token_eof} ) {
			...
			unless ( defined $rv ) {
				PPI::Future::Exception->throw("Error while processing source");
			}
...
sub _process_next_line {
	...
	unless ( $rv ) {
		...
		PPI::Future::Exception->throw("Error at line $self->{line_count}");
	}

	# If we can't deal with the entire line, process char by char
	while ( $rv = $self->_process_next_char ) {}
	unless ( defined $rv ) {
		PPI::Future::Exception->throw("Error at line $self->{line_count}, character $self->{line_cursor}");
	}
...
sub _process_next_char {
	...
	if ( $self->{class} ne "PPI::Future::Token::$result" ) {
		# New class
...
# The defined() in here prevent a ton of calls to PPI::Future::Util::TRUE
sub _new_token {
	my $self = shift;
	...
	defined($self->{token} = $class->new($_[0])) or PPI::Future::Exception->throw;
	$self->{class} = $class;
...
my %OBVIOUS_CLASS = (
	'PPI::Future::Token::Symbol'              => 'operator',
	'PPI::Future::Token::Magic'               => 'operator',
	'PPI::Future::Token::Number'              => 'operator',
	'PPI::Future::Token::ArrayIndex'          => 'operator',
	'PPI::Future::Token::Quote::Double'       => 'operator',
	'PPI::Future::Token::Quote::Interpolate'  => 'operator',
	'PPI::Future::Token::Quote::Literal'      => 'operator',
	'PPI::Future::Token::Quote::Single'       => 'operator',
	'PPI::Future::Token::QuoteLike::Backtick' => 'operator',
	'PPI::Future::Token::QuoteLike::Command'  => 'operator',
	'PPI::Future::Token::QuoteLike::Readline' => 'operator',
	'PPI::Future::Token::QuoteLike::Regexp'   => 'operator',
	'PPI::Future::Token::QuoteLike::Words'    => 'operator',
);
...
sub _opcontext {
	...
	return 'operand' if $p0->isa('PPI::Future::Token::Operator');
...
sub _current_x_is_operator {
	...
	return !$self->__current_token_is_forced_word if $prev->isa('PPI::Future::Token::Word');

	return (!$prev->isa('PPI::Future::Token::Operator') || $X_CAN_FOLLOW_OPERATOR{$prev})
		&& (!$prev->isa('PPI::Future::Token::Structure') || $X_CAN_FOLLOW_STRUCTURE{$prev})
		&& !$prev->isa('PPI::Future::Token::Label')
	;
}
...
For the main part of a document we use L<PPI::Future::Token::Whitespace> for this,
...
You may notice that many methods in the C<PPI::Future::Tokenizer> code look
...
to C, work has started on a L<PPI::Future::XS> "accelerator" package which acts as
a separate and automatically-detected add-on to the main PPI::Future package.

L<PPI::Future::XS> implements faster versions of a variety of functions scattered
over the entire PPI::Future codebase, from the Tokenizer Core, Quote Engine, and
...
function gets out of sync, L<PPI::Future::XS> will degrade gracefully and just
...
- Implement more Tokenizer functions in L<PPI::Future::XS>
