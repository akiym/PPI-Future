package PPI::Future::Exception;

=head1 NAME

PPI::Future::Exception - The PPI::Future exception base class

=head1 SYNOPSIS

  use PPI::Future::Exception;
  
  my $e = PPI::Future::Exception->new( 'something happened' );
  $e->throw;

  PPI::Future::Exception->new( message => 'something happened' )->throw;
  PPI::Future::Exception->throw( message => 'something happened' );

=head1 DESCRIPTION

All exceptions thrown from within PPI::Future will be instances or derivations
of this class.
...
Constructs and returns a new C<PPI::Future::Exception> object.
...
If called on a C<PPI::Future::Exception> object, throws the object.
If called on the class name, uses the arguments to construct a
C<PPI::Future::Exception> and then throw it.
...
sub throw {
	my $it = shift;
	if ( _INSTANCE($it, 'PPI::Future::Exception') ) {
		if ( $it->{callers} ) {
