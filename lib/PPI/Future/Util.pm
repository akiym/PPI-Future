...
# This makes it easy to upgrade many places in PPI::Future::XS
sub TRUE  () { 1  }
sub FALSE () { '' }
...
# Allows a sub that takes a L<PPI::Future::Document> to handle the full range
# of different things, including file names, SCALAR source, etc.
sub _Document {
	shift if @_ > 1;
	return undef unless defined $_[0];
	require PPI::Future::Document;
	return PPI::Future::Document->new(shift) unless ref $_[0];
	return PPI::Future::Document->new(shift) if _SCALAR0($_[0]);
	return PPI::Future::Document->new(shift) if _ARRAY0($_[0]);
	return shift if _INSTANCE($_[0], 'PPI::Future::Document');
	return undef;
}
