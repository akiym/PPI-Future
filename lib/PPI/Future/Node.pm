package PPI::Future::Node;

=pod

=head1 NAME

PPI::Future::Node - Abstract PPI::Future Node class, an Element that can contain other Elements
...
=head1 SYNOPSIS
  ...
  my $Node = PPI::Future::Document->new;
  
  # Add an element to the node( in this case, a token )
  my $Token = PPI::Future::Token::Word->new('my');
  ...
  my $barewords = $Node->find( 'PPI::Future::Token::Word' );
  
  # Find by more complex criteria
  my $my_tokens = $Node->find( sub { $_[1]->content eq 'my' } );
  
  # Remove all the whitespace
  $Node->prune( 'PPI::Future::Token::Whitespace' );
...
The C<PPI::Future::Node> class provides an abstract base class for the Element
classes that are able to contain other elements L<PPI::Future::Document>,
L<PPI::Future::Statement>, and L<PPI::Future::Structure>.

As well as those listed below, all of the methods that apply to
L<PPI::Future::Element> objects also apply to C<PPI::Future::Node> objects.
...
### XS -> PPI::Future/XS.xs:_PPI::Future_Node__scope 0.903+
sub scope() { '' }
...
The C<add_element> method adds a L<PPI::Future::Element> object to the end of a
C<PPI::Future::Node>. Because Elements maintain links to their parent, an
Element can only be added to a single Node.

Returns true if the L<PPI::Future::Element> was added. Returns C<undef> if the
Element was already within another Node, or the method is not passed 
a L<PPI::Future::Element> object.
...
the C<PPI::Future::Node> object. Note that in the base of the L<PPI::Future::Structure>
classes, this C<DOES> include the brace tokens at either end of the
structure.

Returns a list of zero or more L<PPI::Future::Element> objects.
...
the C<PPI::Future::Node> object. As for the C<elements> method, this does include
the brace tokens for L<PPI::Future::Structure> objects.

Returns a L<PPI::Future::Element> object, or C<undef> if for some reason the
C<PPI::Future::Node> object does not contain any elements.
...
C<PPI::Future::Node> object. Note that in the case of the L<PPI::Future::Structure>
classes, this does B<NOT> include the brace tokens at either end of the
structure.

Returns a list of zero of more L<PPI::Future::Element> objects.
...
The C<child> method accesses a child L<PPI::Future::Element> object by its
position within the Node.

Returns a L<PPI::Future::Element> object, or C<undef> if there is no child
...
sub child {
	my ( $self, $index ) = @_;
	PPI::Future::Exception->throw( "method child() needs an index" )
...
such as whitespace and comments, while L<PPI::Future> treats these items as valid
...
The C<contains> method is used to determine if another L<PPI::Future::Element>
object is logically "within" a C<PPI::Future::Node>. For the special case of the
brace tokens at either side of a L<PPI::Future::Structure> object, they are
generally considered "within" a L<PPI::Future::Structure> object, even if they are
not actually in the elements for the L<PPI::Future::Structure>.

Returns true if the L<PPI::Future::Element> is within us, false if not, or C<undef>
on error.
...
L<PPI::Future::Element> objects that meet a particular condition.
...
class name (full or shortened), or a C<CODE>/function reference.
  ...
  $Document->find('PPI::Future::Quote::Single');
  ...
  $Document->find( sub {
  	...
  	and (
  		...
  		$_[1]->isa('PPI::Future::Token::Pod')
  		or
  		$_[1]->isa('PPI::Future::Token::Comment')
  	)
  } );

The function will be passed two arguments, the top-level C<PPI::Future::Node>
you are searching in and the current L<PPI::Future::Element> that the condition
is testing.
...
The C<find> method returns a reference to an array of L<PPI::Future::Element>
...
sub find {
	...
	my $ok = eval {
		while ( @queue ) {
			...
			next unless $Element->isa('PPI::Future::Node');
...
Returns the first L<PPI::Future::Element> object that matches the condition, false
...
sub find_first {
	...
	my $rv;
	my $ok = eval {
		# The defined() here prevents a ton of calls to PPI::Future::Util::TRUE
		while ( @queue ) {
			my $Element = shift @queue;
			...
			# Skip if the Element doesn't have any children
			next if !$Element->isa('PPI::Future::Node');
...
If passed a L<PPI::Future::Element> object that is a direct child of the Node,
...
sub remove_child {
	my $self  = shift;
	my $child = _INSTANCE(shift, 'PPI::Future::Element') or return undef;
...
The C<prune> method is used to strip L<PPI::Future::Element> objects out of a code
...
sub prune {
	...
	my $ok = eval {
		while ( my $element = shift @queue ) {
			...
			if ( _INSTANCE($element, 'PPI::Future::Node') ) {
...
###       break File::Find::Rule::PPI::Future
sub _wanted {
	my $either = shift;
...
####################################################################
# PPI::Future::Element overloaded methods
...
sub __link_children {
	...
	while ( my $Node = shift @queue ) {
		...
		foreach my $Element ( @{$Node->{children}} ) {
			...
			unshift @queue, $Element if $Element->isa('PPI::Future::Node');
		}

		# If it's a structure, relink the open/close braces
		next unless $Node->isa('PPI::Future::Structure');
		Scalar::Util::weaken(
...
- Move as much as possible to L<PPI::Future::XS>
