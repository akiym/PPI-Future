package PPI::Future::Document;

=pod

=head1 NAME

PPI::Future::Document - Object representation of a Perl document
...
=head1 SYNOPSIS
  ...
  my $Document = PPI::Future::Document->new('My/Module.pm');
  ...
  my $sub_nodes = $Document->find( 
  	sub { $_[1]->isa('PPI::Future::Statement::Sub') and $_[1]->name }
  );
...
The C<PPI::Future::Document> class represents a single Perl "document". A
C<PPI::Future::Document> object acts as a root L<PPI::Future::Node>, with some
additional methods for loading and saving, and working with
the line/column locations of Elements within a file.

The exemption to its L<PPI::Future::Node>-like behavior this is that a
C<PPI::Future::Document> object can NEVER have a parent node, and is always
the root node in a tree.

=head2 Storable Support

C<PPI::Future::Document> implements the necessary C<STORABLE_freeze> and
...
probably going to involve the methods from L<PPI::Future::Node> class, of which
this is a subclass.
...
our ( $errstr, @ISA ) = ( "", "PPI::Future::Node" );
...
=head2 new
  ...
  # Simple construction
  $doc = PPI::Future::Document->new( $filename );
  $doc = PPI::Future::Document->new( \$source  );
...
Perl code, and creates a single cohesive Perl C<PPI::Future::Document>
for it.
...
to where it was created from. Specifically, if you create a PPI::Future::Document from
...
Returns a C<PPI::Future::Document> object, or C<undef> if parsing fails.
L<PPI::Future::Exception> objects can also be thrown if there are parsing problems.
...
sub new {
	...
	if ( ! defined $source ) {
		$class->_error("An undefined value was passed to PPI::Future::Document::new");

	} elsif ( ! ref $source ) {
		# Catch people using the old API
		if ( $source =~ /(?:\012|\015)/ ) {
			Carp::croak("API CHANGE: Source code should only be passed to PPI::Future::Document->new as a SCALAR reference");
		}

		# When loading from a filename, use the caching layer if it exists.
		if ( $CACHE ) {
			my $file_contents = PPI::Future::Util::_slurp( $source );
			...
			$document = PPI::Future::Lexer->lex_source( $$file_contents );
			if ( $document ) {
				# Save in the cache
			...
			my $document = PPI::Future::Lexer->lex_file( $source );
			return $class->_setattr( $document, %attr ) if $document;
		}

	} elsif ( _SCALAR0($source) ) {
		my $document = PPI::Future::Lexer->lex_source( $$source );
		return $class->_setattr( $document, %attr ) if $document;
		...
		$class->_error("Unknown object or reference was passed to PPI::Future::Document::new");
	}
	...
	if ( _INSTANCE($@, 'PPI::Future::Exception') ) {
		...
		$errstr = $@;
		$errstr =~ s/\sat line\s.+$//;
	} elsif ( PPI::Future::Lexer->errstr ) {
		$errstr = PPI::Future::Lexer->errstr;
	} else {
		$errstr = "Unknown error parsing Perl document";
	}
	PPI::Future::Lexer->_clear;
	$class->_error( $errstr );
}

sub load {
	Carp::croak("API CHANGE: File names should now be passed to PPI::Future::Document->new to load a file");
}
...
As of L<PPI::Future> 1.100, C<PPI::Future::Document> supports parser caching.

The default cache class L<PPI::Future::Cache> provides a L<Storable>-based
...
C<PPI::Future::Document> to use when loading documents. It takes as argument
a L<PPI::Future::Cache> object (or something that C<isa> the same).

If passed C<undef>, this method will stop using the current cache, if any.

For more information on caching, see L<PPI::Future::Cache>.
...
sub set_cache {
	...
	if ( defined $_[0] ) {
		# Enable the cache
		my $object = _INSTANCE(shift, 'PPI::Future::Cache') or return undef;
		$CACHE = $object;
	} else {
		# Disable the cache
		$CACHE = undef;
...
Returns a L<PPI::Future::Cache> object, or C<undef> if there is no cache
currently set for C<PPI::Future::Document>.
...
#####################################################################
# PPI::Future::Document Instance Methods
...
writers of L<PPI::Future>-related systems may or may not use to enable
...
At the present time, PPI::Future only supports "naive" (width 1) tabs, but we do
...
The C<save> method serializes the C<PPI::Future::Document> object and saves the
...
sub serialize {
	...
	foreach my $i ( 0 .. $#tokens ) {
		...
		unless ( $Token->isa('PPI::Future::Token::HereDoc') ) {
		...
		if ( $Token->{_damaged} ) {
			...
			my $any_after = List::Util::any {
				$tokens[$_]->isa('PPI::Future::Token::HereDoc')
				and (
...
This identifier is used by a variety of systems (such as L<PPI::Future::Cache>
...
sub hex_id {
	PPI::Future::Util::md5hex($_[0]->serialize);
}
...
Within a document, all L<PPI::Future::Element> objects can be considered to have a
...
advance, making future calls to <PPI::Future::Element::location> virtually free.
...
sub index_locations {
	...
	if ( defined $first ) {
		foreach ( $first .. $#tokens ) {
			...
			if ( $Token->isa('PPI::Future::Token::HereDoc') ) {
...
sub _logical_line_and_file {
	...
	if ($start->[LOCATION_CHARACTER] == 1) {
		if ( $Token->isa('PPI::Future::Token::Comment') ) {
			if (
				$Token->content =~ m<
					\A
					\#      \s*
		...
		elsif ( $Token->isa('PPI::Future::Token::Pod') ) {
			my $content = $Token->content;
			my $line;
...
L<PPI::Future::Document::Normalized> object for the current Document.
...
See L<PPI::Future::Normal> for more information on document normalization and
the tasks for which it is useful.

Returns a L<PPI::Future::Document::Normalized> object, or C<undef> on error.
...
sub normalized {
	...
	# clone of ourselves.
	PPI::Future::Normal->process( $_[0]->clone );
}
...
sub complete {
	...
	while ( @child and not $child[-1]->isa('PPI::Future::Statement') ) {
		pop @child;
	}
...
### XS -> PPI::Future/XS.xs:_PPI::Future_Document__scope 0.903+
sub scope() { 1 }
...
sub insert_before {
	return undef;
	# die "Cannot insert_before a PPI::Future::Document";
}

sub insert_after {
	return undef;
	# die "Cannot insert_after a PPI::Future::Document";
}

sub replace {
	return undef;
	# die "Cannot replace a PPI::Future::Document";
}
