package PPI::Future::Find;

=pod

=head1 NAME

PPI::Future::Find - Object version of the Element->find method

=head1 SYNOPSIS

  # Create the Find object
  my $Find = PPI::Future::Find->new( \&wanted );
...
PPI::Future::Find is the primary PDOM searching class in the core PPI::Future package.

=head2 History

It became quite obvious during the development of PPI::Future that many of the
...
it is by no means thorough. PPI::Future::Find attempts to resolve this problem.

=head2 Structure and Style

PPI::Future::Find provides a similar API to the popular L<File::Find::Rule>
module for file searching, but without the ability to assemble queries.

The implementation of a separate PPI::Future::Find::Rule sub-class that does
provide this ability is left as an exercise for the reader.

=head2 The &wanted function

At the core of each PPI::Future::Find object is a "wanted" function that is
...
Returns a new PPI::Future::Find object, or C<undef> if not passed a CODE reference.
...
Returns a duplicate PPI::Future::Find object.
...
It takes as argument a single L<PPI::Future::Element> object which will
serve as the top of the search process.

Returns a list of PPI::Future::Element objects that match the condition
...
sub in {
	...
	unless ( _INSTANCE($Element, 'PPI::Future::Element') ) {
		return $self->_error('->in was not passed a PPI::Future::Element object', %params);
	}
...
is passed the parent PPI::Future::Element object as for the C<in> method, but does
...
sub start {
	...
	# Get the root element for the search
	unless ( _INSTANCE($Element, 'PPI::Future::Element') ) {
		return $self->_error('->in was not passed a PPI::Future::Element object');
	}
...
Returns a PPI::Future::Element object, or C<undef> if there are no remaining
...
The C<errstr> method returns the error messages when a given PPI::Future::Find
...
- Implement the L<PPI::Future::Find::Rule> class
