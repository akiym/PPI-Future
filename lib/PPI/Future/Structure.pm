package PPI::Future::Structure;

=pod

=head1 NAME

PPI::Future::Structure - The base class for Perl braced structures
...
PPI::Future::Structure is the root class for all Perl bracing structures. This
covers all forms of C< [ ... ] >, C< { ... } >, and C< ( ... ) > brace
types, and includes cases where only one half of the pair exist.

The class PPI::Future::Structure itself is full abstract and no objects of that
type should actually exist in the tree.

=head2 Elements vs Children

A B<PPI::Future::Structure> has an unusual existence. Unlike a L<PPI::Future::Document>
or L<PPI::Future::Statement>, which both simply contain other elements, a
...
Excluding the transient L<PPI::Future::Structure::Unknown> that exists briefly
inside the parser, there are eight types of structure.

=head2 L<PPI::Future::Structure::List>
...
=head2 L<PPI::Future::Structure::For>
...
C<PPI::Future::Structure> itself has very few methods. Most of the time, you will be
...
use PPI::Future::Structure::Block       ();
use PPI::Future::Structure::Condition   ();
use PPI::Future::Structure::Constructor ();
use PPI::Future::Structure::For         ();
use PPI::Future::Structure::Given       ();
use PPI::Future::Structure::List        ();
use PPI::Future::Structure::Subscript   ();
use PPI::Future::Structure::Unknown     ();
use PPI::Future::Structure::When        ();
...
sub new {
	my $class = shift;
	my $Token = PPI::Future::Token::__LEXER__opens($_[0]) ? shift : return undef;
...
#####################################################################
# PPI::Future::Structure API methods
...
braces for the structure are known within PPI::Future as the "start" and "finish"
braces (at least for method purposes).

The C<start> method returns the start brace for the structure (i.e. the
opening brace).

Returns the brace as a L<PPI::Future::Token::Structure> or C<undef> if the
...
#####################################################################
# PPI::Future::Node overloaded methods
