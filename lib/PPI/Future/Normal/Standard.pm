package PPI::Future::Normal::Standard;

=pod

=head1 NAME

PPI::Future::Normal::Standard - Provides standard document normalization functions

=head1 DESCRIPTION

This module provides the default normalization methods for L<PPI::Future::Normal>.
...
sub import {
	PPI::Future::Normal->register(
		map { /\D/ ? "PPI::Future::Normal::Standard::$_" : $_ } @METHODS
	) or die "Failed to register PPI::Future::Normal::Standard transforms";
}
...
sub remove_useless_pragma {
	my $Document = shift;
	$Document->prune( sub {
		return '' unless $_[1]->isa('PPI::Future::Statement::Include');
...
sub remove_statement_separator {
	my $Document = shift;
	$Document->prune( sub {
		$_[1]->isa('PPI::Future::Token::Structure') or return '';
		$_[1]->content eq ';'               or return '';
		my $stmt = $_[1]->parent            or return '';
		$stmt->isa('PPI::Future::Statement')        or return '';
...
sub remove_useless_return {
	my $Document = shift;
	$Document->prune( sub {
		$_[1]->isa('PPI::Future::Token::Word')       or return '';
		$_[1]->content eq 'return'           or return '';
		my $stmt = $_[1]->parent             or return '';
		$stmt->isa('PPI::Future::Statement::Break')  or return '';
		$stmt->children == 2                 or return '';
		$stmt->next_sibling                 and return '';
		my $block = $stmt->parent            or return '';
		$block->isa('PPI::Future::Structure::Block') or return '';
		1;
	} );
}
