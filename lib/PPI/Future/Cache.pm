package PPI::Future::Cache;

=pod

=head1 NAME

PPI::Future::Cache - The PPI::Future Document Caching Layer

=head1 SYNOPSIS

  # Set the cache
  use PPI::Future::Cache path => '/var/cache/ppi-cache';
  
  # Manually create a cache
  my $Cache = PPI::Future::Cache->new(
      path     => '/var/cache/perl/class-PPI::Future',
      readonly => 1,
  );

=head1 DESCRIPTION

C<PPI::Future::Cache> provides the default caching functionality for L<PPI::Future>.

It integrates automatically with L<PPI::Future> itself. Once enabled, any attempt
to load a document from the filesystem will be cached via cache.

Please note that creating a L<PPI::Future::Document> from raw source or something
other object will B<not> be cached.

=head2 Using PPI::Future::Cache

The most common way of using C<PPI::Future::Cache> is to provide parameters to
...
sub import {
	...
	# Make PPI::Future::Document use it
	unless ( PPI::Future::Document->set_cache( $cache ) ) {
		Carp::croak("Failed to set cache in PPI::Future::Document");
	}
...
Returns a new C<PPI::Future::Cache> object, or dies on error.
...
sub new {
	...
	my $path = $params{path}
		or Carp::croak("Cannot create PPI::Future::Cache, no path provided");
	unless ( -d $path ) {
		Carp::croak("Cannot create PPI::Future::Cache, path does not exist");
	}
	unless ( -r $path and -x $path ) {
		Carp::croak("Cannot create PPI::Future::Cache, no read permissions for path");
	}
	if ( ! $params{readonly} and ! -w $path ) {
		Carp::croak("Cannot create PPI::Future::Cache, no write permissions for path");
	}
...
sub get_document {
	my $self = ref $_[0]
		? shift
		: Carp::croak('PPI::Future::Cache::get_document called as static method');
...
The C<store_document> method takes a L<PPI::Future::Document> as argument and
...
# Store an arbitrary PPI::Future::Document object (using Storable) to a particular
...
sub _load {
	...
	# Security check
	unless ( _INSTANCE($object, 'PPI::Future::Document') ) {
		Carp::croak("Security Violation: Object in '$file' is not a PPI::Future::Document");
	}
...
- Add support for use PPI::Future::Cache auto-setting $PPI::Future::Document::CACHE
