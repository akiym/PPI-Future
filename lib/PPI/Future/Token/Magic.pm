package PPI::Future::Token::Magic;

=pod

=head1 NAME

PPI::Future::Token::Magic - Tokens representing magic variables

=head1 INHERITANCE

  PPI::Future::Token::Magic
  isa PPI::Future::Token::Symbol
...
C<PPI::Future::Token::Magic> is a sub-class of L<PPI::Future::Token::Symbol> which
...
L<PPI::Future::Token::Symbol>, L<PPI::Future::Token> and L<PPI::Future::Element>.
...
our @ISA = "PPI::Future::Token::Symbol";
...
sub __TOKENIZER__on_char {
	...
	if ( $c =~ /^  \$  .*  [  \w  :  \$  \{  ]  $/x ) {
		...
		if ( $c =~ /^(\$(?:\_[\w:]|::))/ or $c =~ /^\$\'[\w]/ ) {
			...
			return PPI::Future::Token::Symbol->__TOKENIZER__on_char( $t );
		}

		if ( $c =~ /^\$\$\w/ ) {
			# This is really a scalar dereference. ( $$foo )
			# Add the current token as the cast...
			$t->{token} = PPI::Future::Token::Cast->new( '$' );
			$t->_finalize_token;
		...
		if ( $c =~ /^(\$\#)\w/ ) {
			...
			$t->{token} = PPI::Future::Token::ArrayIndex->new( "$1" );
			return PPI::Future::Token::ArrayIndex->__TOKENIZER__on_char( $t );
		}
		...
		if ($MAGIC{$c}) {
			...
			bless $t->{token}, $t->{class} = 'PPI::Future::Token::Operator';
			$t->{line_cursor}--;
		}
	}
