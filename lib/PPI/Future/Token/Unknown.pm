package PPI::Future::Token::Unknown;

=pod

=head1 NAME

PPI::Future::Token::Unknown - Token of unknown or as-yet undetermined type
...
Object of the type C<PPI::Future::Token::Unknown> exist primarily inside the
...
Please report any C<PPI::Future::Token::Unknown> you encounter in a L<PPI::Future::Document>
object as a bug.
...
use PPI::Future::Singletons qw' %MAGIC $CURLY_SYMBOL ';
...
sub __TOKENIZER__on_char {
	...
	if ( $c eq '*' ) {
		...
		if ( $char =~ /[\w:]/ ) {
			...
			if ( not $prev or not $prev->isa('PPI::Future::Token::Number') ) {
		...
		if ( $char eq '*' ) {
			my ( $prev ) = $t->_previous_significant_tokens(1);
			if ( $prev and $prev->isa('PPI::Future::Token::Operator') and $prev->content eq '->' ) {
	...
	# erm...
	PPI::Future::Exception->throw('Unknown value in PPI::Future::Token::Unknown token');
}
...
sub _cast_or_op {
	...
	if ( $token->isa( 'PPI::Future::Token::Structure' ) and $token->content eq '}' ) {
		...
		( $token, $cursor ) = _prev_significant_w_cursor(
			...
			sub {
				my ( $token ) = @_;
				return if !$token->isa( 'PPI::Future::Token::Structure' );
				if ( $token eq '}' ) {
					$structure_depth++;
					return;
				}
		...
		return 'Operator' if $token->isa( 'PPI::Future::Token::Symbol' ); # subscript
		...
		my $produces_or_wants_value =
		  ( $token->isa( 'PPI::Future::Token::Word' ) and ( $content eq 'do' or $content eq 'eval' ) );
		return $produces_or_wants_value ? 'Operator' : 'Cast';
	}

	my %list_start_or_term_end = map { $_ => 1 } qw@ ; ( { [ @;
	return 'Cast'
	  if $token->isa( 'PPI::Future::Token::Structure' ) and $list_start_or_term_end{ $token->content }
	  or $token->isa( 'PPI::Future::Token::Cast' )
	  or $token->isa( 'PPI::Future::Token::Operator' )
	  or $token->isa( 'PPI::Future::Token::Label' );

	return 'Operator' if !$token->isa( 'PPI::Future::Token::Word' );
...
sub __TOKENIZER__is_an_attribute {
	...
	return 1 if $p0->isa('PPI::Future::Token::Attribute');

	# If we just had a prototype, then we are an attribute
	return 1 if $p0->isa('PPI::Future::Token::Prototype');

	# Other than that, we would need to have had a bareword
	return '' unless $p0->isa('PPI::Future::Token::Word');

	# We could be an anonymous subroutine
	if ( $p0->isa('PPI::Future::Token::Word') and $p0->content eq 'sub' ) {
		return 1;
	}
