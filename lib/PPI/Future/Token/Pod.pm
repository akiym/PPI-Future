package PPI::Future::Token::Pod;

=pod

=head1 NAME

PPI::Future::Token::Pod - Sections of POD in Perl documents
...
A single C<PPI::Future::Token::Pod> object represents a complete section of POD
documentation within a Perl document.

=head1 METHODS

This class provides some additional methods beyond those provided by its
L<PPI::Future::Token> and L<PPI::Future::Element> parent classes.
...
#####################################################################
# PPI::Future::Token::Pod Methods

=pod

=head2 merge @podtokens

The C<merge> constructor takes a number of C<PPI::Future::Token::Pod> objects,
and returns a new object that represents one combined POD block with
the content of all of them.

Returns a new C<PPI::Future::Token::Pod> object, or C<undef> on error.
...
sub merge {
	...
	if ( grep { ! _INSTANCE($_, 'PPI::Future::Token::Pod') } @_ ) {
		return undef;
	}
...
### XS -> PPI::Future/XS.xs:_PPI::Future_Token_Pod__significant 0.900+
sub significant() { '' }
