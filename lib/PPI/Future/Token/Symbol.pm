package PPI::Future::Token::Symbol;

=pod

=head1 NAME

PPI::Future::Token::Symbol - A token class for variables and other symbols
...
The C<PPI::Future::Token::Symbol> class is used to cover all tokens that represent
...
#####################################################################
# PPI::Future::Token::Symbol Methods
...
sub symbol {
	...
	return $symbol unless _INSTANCE($after, 'PPI::Future::Structure');
	...
	if ( $type eq '$' ) {
		...
		return $symbol if $before &&
			$before->isa( 'PPI::Future::Token::Cast' ) &&
