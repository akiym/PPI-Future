package PPI::Future::Token::Structure;

=pod

=head1 NAME

PPI::Future::Token::Structure - Token class for characters that define code structure
...
The C<PPI::Future::Token::Structure> class is used for tokens that control the
...
sub next_token {
	...
	if ( $OPENS{ ord $self->{content} } ) {
		...
		if ( $child ) {
			...
			return $child->isa('PPI::Future::Token') ? $child : $child->first_token;
...
sub previous_token {
	...
	if ( $CLOSES{ ord $self->{content} } ) {
		...
		if ( $child ) {
			# Decend deeper, or return if it is a token
			return $child->isa('PPI::Future::Token') ? $child : $child->last_token;
