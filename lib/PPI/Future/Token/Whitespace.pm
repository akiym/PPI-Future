package PPI::Future::Token::Whitespace;

=pod

=head1 NAME

PPI::Future::Token::Whitespace - Tokens representing ordinary white space
...
As a full "round-trip" parser, PPI::Future records every last byte in a
file and ensure that it is included in the L<PPI::Future::Document> object.
...
contain vast quantities of C<PPI::Future::Token::Whitespace> objects.
...
series of L<PPI::Future::Node> and L<PPI::Future::Element> methods to do things.
...
provided by the parent L<PPI::Future::Token> and L<PPI::Future::Element> classes.
...
Because L<PPI::Future> sees documents as sitting on a sort of substrate made of
...
### XS -> PPI::Future/XS.xs:_PPI::Future_Token_Whitespace__significant 0.900+
sub significant() { '' }
...
in its own L<PPI::Future::Token::Pod> object.
...
my %COMMITMAP = (
	map( { ord $_ => 'PPI::Future::Token::Word' } 'a' .. 'u', 'A' .. 'Z', qw" w y z _ " ),    # no v or x
	map( { ord $_ => 'PPI::Future::Token::Structure' } qw" ; [ ] { } ) " ),
	ord '#' => 'PPI::Future::Token::Comment',
	ord 'v' => 'PPI::Future::Token::Number::Version',
);
my %CLASSMAP = (
	map( { ord $_ => 'Number' } 0 .. 9 ),
	map( { ord $_ => 'Operator' } qw" = ? | + > . ! ~ ^ " ),
	map( { ord $_ => 'Unknown' } qw" * $ @ & : % " ),
	ord ','  => 'PPI::Future::Token::Operator',
	ord "'"  => 'Quote::Single',
	ord '"'  => 'Quote::Double',
...
sub __TOKENIZER__on_char {
	...
	if ( $char == 40 ) {  # $char eq '('
		...
		if (
			$tokens[0]
			and
			$tokens[0]->isa('PPI::Future::Token::Word')
			and
			$p1
			and
			$p1->isa('PPI::Future::Token::Word')
			and
			$p1->content eq 'sub'
			and (
				not $p2
				or
				$p2->isa('PPI::Future::Token::Structure')
				or (
					$p2->isa('PPI::Future::Token::Whitespace')
					and
					$p2->content eq ''
				)
			)
		...
		my $p0 = $tokens[0];
		if ( $p0 and $p0->isa('PPI::Future::Token::Word') and $p0->content eq 'sub'
			# Maybe it's invoking a method named 'sub'
			and not ( $p1 and $p1->isa('PPI::Future::Token::Operator') and $p1->content eq '->')
		) {
			return 'Prototype';
		}
		...
		# 1 < $bar
		# $#foo < $bar
		return 'Operator' if $prev and $prev->isa('PPI::Future::Token::Symbol');
		return 'Operator' if $prev and $prev->isa('PPI::Future::Token::Magic');
		return 'Operator' if $prev and $prev->isa('PPI::Future::Token::Number');
		return 'Operator' if $prev and $prev->isa('PPI::Future::Token::ArrayIndex');
		...
		if ( $prev->isa('PPI::Future::Token::Structure') and $prec eq '(' ) {
			return 'QuoteLike::Readline';
		}
		if ( $prev->isa('PPI::Future::Token::Word') and $prec eq 'while' ) {
			return 'QuoteLike::Readline';
		}
		if ( $prev->isa('PPI::Future::Token::Operator') and $prec eq '=' ) {
			return 'QuoteLike::Readline';
		}
		if ( $prev->isa('PPI::Future::Token::Operator') and $prec eq ',' ) {
			return 'QuoteLike::Readline';
		...
		# =~ - A bound regex
		# !~ - Ditto
		return 'Regexp::Match' if $prev->isa('PPI::Future::Token::Operator');

		# After a symbol
		return 'Operator' if $prev->isa('PPI::Future::Token::Symbol');
		if ( $prec eq ']' and $prev->isa('PPI::Future::Token::Structure') ) {
			return 'Operator';
		}

		# After another number
		return 'Operator' if $prev->isa('PPI::Future::Token::Number');

		# After going into scope/brackets
		if (
			$prev->isa('PPI::Future::Token::Structure')
			and (
				$prec eq '('
				or
				$prec eq '{'
		...
		if (
			$MATCHWORD{$prec}
			and
			$prev->isa('PPI::Future::Token::Word')
		) {
			return 'Regexp::Match';
		}
		...
		return PPI::Future::Token::Word->__TOKENIZER__commit($t);
		...
		return 'PPI::Future::Token::Word'->__TOKENIZER__commit($t) if $c =~ /\w/;
	...
	### For now, die
	PPI::Future::Exception->throw("Encountered unexpected character '$char'");
}
