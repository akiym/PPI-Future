package PPI::Future::Token::Word;

=pod

=head1 NAME

PPI::Future::Token::Word - The generic "word" Token
...
A C<PPI::Future::Token::Word> object is a PPI::Future-specific representation of several
...
There are no methods available for C<PPI::Future::Token::Word> beyond those
provided by its L<PPI::Future::Token> and L<PPI::Future::Element> parent
classes.
...
sub method_call {
	...
	if (
		$previous
		and
		$previous->isa('PPI::Future::Token::Operator')
		and
		$previous->content eq '->'
	) {
		return 1;
	...
	if (
		$snext->isa('PPI::Future::Structure::List')
		or
		$snext->isa('PPI::Future::Token::Structure')
		or
		$snext->isa('PPI::Future::Token::Operator')
		and (
			$snext->content eq ','
			or
			$snext->content eq '=>'
	...
	if (
		$snext->isa('PPI::Future::Token::Word')
		and
...
sub __current_token_is_attribute {
	...
	return (
		...
		and (
			...
			or $tokens[0]->isa('PPI::Future::Token::Attribute')
		)
	);
}
