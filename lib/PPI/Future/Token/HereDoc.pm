package PPI::Future::Token::HereDoc;

=pod

=head1 NAME

PPI::Future::Token::HereDoc - Token class for the here-doc
...
Firstly, the PPI::Future::Token::HereDoc object, does not represent the C<<< << >>>
...
To make things work in the way you expect, PPI::Future has to play some games
...
Please note that due to the immature nature of PPI::Future in general, we expect
...
and will not be found in a search for L<PPI::Future::Token::Quote> or
L<PPI::Future::Token::QuoteLike objects>.
...
#####################################################################
# PPI::Future::Token::HereDoc Methods
...
sub __TOKENIZER__on_char {
	...
	if ( @heredoc and defined $heredoc[-1] ) {
		# See PPI::Future::Tokenizer, the algorithm there adds a space at the end of the
	...
	# Set a hint for PPI::Future::Document->serialize so it can
...
- Implement PPI::Future::Token::Quote interface compatibility
