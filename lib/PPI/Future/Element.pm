package PPI::Future::Element;

=pod

=head1 NAME

PPI::Future::Element - The abstract Element class, a base for all source objects

=head1 INHERITANCE

  PPI::Future::Element is the root of the PDOM tree

=head1 DESCRIPTION

The abstract C<PPI::Future::Element> serves as a base class for all source-related
...
use overload 'bool' => \&PPI::Future::Util::TRUE;
...
be able to "round trip" the L<PPI::Future::Document> back to a file) the
...
### XS -> PPI::Future/XS.xs:_PPI::Future_Element__significant 0.845+
sub significant() { 1 }
...
The C<tokens> method returns a list of L<PPI::Future::Token> objects for the
...
For B<any> C<PPI::Future::Element>, the C<content> method will reconstitute the
...
B<not> eval or execute the result if it contains any L<PPI::Future::Token::HereDoc>.

The L<PPI::Future::Document> method C<serialize> should be used to stringify a PDOM
...
### XS -> PPI::Future/XS.xs:_PPI::Future_Element__content 0.900+
sub content() { '' }
...
left to the L<PPI::Future::Node> abstract class, a subclass of C<PPI::Future::Element>.
...
Answers whether a C<PPI::Future::Element> is contained within another one.

C<PPI::Future::Element>s are considered to be descendants of themselves.
...
Answers whether a C<PPI::Future::Element> is contains another one.

C<PPI::Future::Element>s are considered to be ancestors of themselves.
...
For a C<PPI::Future::Element> that is contained (at some depth) within a
L<PPI::Future::Statement>, the C<statement> method will return the first parent
Statement object lexically 'above' the Element.

Returns a L<PPI::Future::Statement> object, which may be the same Element if the
Element is itself a L<PPI::Future::Statement> object.
...
sub statement {
	my $cursor = shift;
	while ( ! _INSTANCE($cursor, 'PPI::Future::Statement') ) {
		$cursor = $_PARENT{refaddr $cursor} or return '';
	}
	$cursor;
}
...
For a C<PPI::Future::Element> that is contained within a PDOM tree, the C<top> method
will return the top-level Node in the tree. Most of the time this should be
a L<PPI::Future::Document> object, however this will not always be so. For example,
...
For an Element that is contained within a L<PPI::Future::Document> object,
the C<document> method will return the top-level Document for the Element.

Returns the L<PPI::Future::Document> for this Element, or false if the Element is not
...
sub document {
	my $top = shift->top;
	_INSTANCE($top, 'PPI::Future::Document') and $top;
}
...
All L<PPI::Future::Node> objects (specifically, our parent Node) contain a number of
C<PPI::Future::Element> objects. The C<next_sibling> method returns the C<PPI::Future::Element>
...
B<significant> sibling of the C<PPI::Future::Element> object.

Returns a C<PPI::Future::Element> object, or false if there is no 'next' significant
sibling.
...
All L<PPI::Future::Node> objects (specifically, our parent Node) contain a number of
C<PPI::Future::Element> objects. The C<previous_sibling> method returns the Element
immediately before the current one, or false if there is no 'previous'
C<PPI::Future::Element> object.
...
the previous B<significant> sibling of the C<PPI::Future::Element> object.

Returns a C<PPI::Future::Element> object, or false if there is no 'previous' significant
sibling.
...
PPI::Future::Token object within or equal to this one.

That is, if called on a L<PPI::Future::Node> subclass, it will descend until it
finds a L<PPI::Future::Token>. If called on a L<PPI::Future::Token> object, it will return
the same object.

Returns a L<PPI::Future::Token> object, or dies on error (which should be extremely
rare and only occur if an illegal empty L<PPI::Future::Statement> exists below the
...
sub first_token {
	my $cursor = shift;
	while ( $cursor->isa('PPI::Future::Node') ) {
		$cursor = $cursor->first_element
		or die "Found empty PPI::Future::Node while getting first token";
	}
	$cursor;
}
...
L<PPI::Future::Token> object that is immediately after the current Element, even if
it is not within the same parent L<PPI::Future::Node> as the one for which the
method is being called.

Note that this is B<not> defined as a L<PPI::Future::Token>-specific method,
because it can be useful to find the next token that is after, say, a
L<PPI::Future::Statement>, although obviously it would be useless to want the
next token after a L<PPI::Future::Document>.

Returns a L<PPI::Future::Token> object, or false if there are no more tokens after
the Element.
...
sub next_token {
	...
	while ( 1 ) {
		...
		if ( $element ) {
			return $element if $element->isa('PPI::Future::Token');
			return $element->first_token;
		}
		$cursor = $cursor->parent or return '';
		if ( $cursor->isa('PPI::Future::Structure') and $cursor->finish ) {
...
L<PPI::Future::Token> object that is immediately before the current Element, even
if it is not within the same parent L<PPI::Future::Node> as this one.

Note that this is not defined as a L<PPI::Future::Token>-only method, because it can
be useful to find the token is before, say, a L<PPI::Future::Statement>, although
obviously it would be useless to want the next token before a
L<PPI::Future::Document>.

Returns a L<PPI::Future::Token> object, or false if there are no more tokens before
the C<Element>.
...
sub previous_token {
	...
	while ( 1 ) {
		...
		$cursor = $cursor->parent or return '';
		if ( $cursor->isa('PPI::Future::Structure') and $cursor->start ) {
			return $cursor->start;
		}
	}
}
...
the form of C<PPI::Future::Element> objects, before the calling C<Element>. You
...
Elements, inline-parsed code strings or L<PPI::Future::Document::Fragment> objects.

Returns true if the Element was inserted, false if it can not be inserted,
or C<undef> if you do not provide a C<PPI::Future::Element> object as a parameter.
...
the form of C<PPI::Future::Element> objects, after the calling C<Element>. You need
...
For a given C<PPI::Future::Element>, the C<remove> method will remove it from its
...
For a given C<PPI::Future::Element>, the C<delete> method will remove it from its
...
If the Element exists within a L<PPI::Future::Document> that has
indexed the Element locations using C<PPI::Future::Document::index_locations>, the
...
into account tabbing (see L<PPI::Future::Document/"tab_width [ $width ]">).
...
Returns C<undef> on error, or if the L<PPI::Future::Document> object has not been
indexed.
...
If the Element exists within a L<PPI::Future::Document> that has indexed the Element
locations using C<PPI::Future::Document::index_locations>, the C<line_number> method
...
If the Element exists within a L<PPI::Future::Document> that has indexed the Element
locations using C<PPI::Future::Document::index_locations>, the C<column_number> method
...
If the Element exists within a L<PPI::Future::Document> that has indexed the Element
locations using C<PPI::Future::Document::index_locations>, the C<visual_column_number>
...
L<PPI::Future::Document/"tab_width [ $width ]">.
...
If the Element exists within a L<PPI::Future::Document> that has indexed the Element
locations using C<PPI::Future::Document::index_locations>, the C<logical_line_number>
...
If the Element exists within a L<PPI::Future::Document> that has indexed the Element
locations using C<PPI::Future::Document::index_locations>, the C<logical_filename>
...
sub _ensure_location_present {
	...
	unless ( exists $self->{_location} ) {
		...
		if ( $Document->isa('PPI::Future::Document::Fragment') ) {
...
sub _flush_locations {
	...
	if ( _INSTANCE($_[0], 'PPI::Future::Element') ) {
...
### XS -> PPI::Future/XS.xs:_PPI::Future_Element__DESTROY 0.900+
...
sub __eq {
	my $self  = _INSTANCE($_[0], 'PPI::Future::Element') ? $_[0]->content : $_[0];
	my $other = _INSTANCE($_[1], 'PPI::Future::Element') ? $_[1]->content : $_[1];
	$self eq $other;
}
sub __ne { !__eq(@_) }
