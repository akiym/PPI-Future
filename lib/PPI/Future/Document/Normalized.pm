package PPI::Future::Document::Normalized;

=pod

=head1 NAME

PPI::Future::Document::Normalized - A normalized Perl Document

=head1 DESCRIPTION

A C<Normalized Document> object is the result of the normalization process
contained in the L<PPI::Future::Normal> class. See the documentation for
L<PPI::Future::Normal> for more information.

The object contains a version stamp and function list for the version
of L<PPI::Future::Normal> used to create it, and a processed and delinked
L<PPI::Future::Document> object.
...
true if they were produced by the same version of PPI::Future::Normal, with the
...
The C<new> method is intended for use only by the L<PPI::Future::Normal> class,
and to get ::Normalized objects, you are highly recommended to use
either that module, or the C<normalized> method of the L<PPI::Future::Document>
object itself.
...
sub new {
	...
	my $Document  = _INSTANCE($args{Document}, 'PPI::Future::Document') or return undef;
...
The C<version> accessor returns the L<PPI::Future::Normal> version used to create
the object.
...
PPI::Future::Document::Normalized object, and checking for equivalence to it.

The C<==> operator is also overload to this method, so that you can
do something like the following:

  my $first  = PPI::Future::Document->load('first.pl');
  my $second = PPI::Future::Document->load('second.pl');
...
sub equal {
	my $self  = shift;
	my $other = _INSTANCE(shift, 'PPI::Future::Document::Normalized') or return undef;
