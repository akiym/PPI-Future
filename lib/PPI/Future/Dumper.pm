package PPI::Future::Dumper;

=pod

=head1 NAME

PPI::Future::Dumper - Dumping of PDOM trees

=head1 SYNOPSIS

  # Load a document
  my $Module = PPI::Future::Document->new( 'MyModule.pm' );
  
  # Create the dumper
  my $Dumper = PPI::Future::Dumper->new( $Module );
...
The PDOM trees in PPI::Future are quite complex, and getting a dump of their
...
L<PPI::Future::Element> object of any type to serve as the root of the tree to
...
Returns a new C<PPI::Future::Dumper> object, or C<undef> if the constructor
is not passed a correct L<PPI::Future::Element> root object.
...
[ line, rowchar, column ]. See L<PPI::Future::Element/"location"> for a description of
...
sub new {
	...
	# Handle the options
	my @options = map { lc $_ } @_; # strict hashpairs # https://github.com/adamkennedy/PPI::Future/issues/201
	my %options = @options;
...
sub _dump {
	...
	my $Element = _INSTANCE($_[0], 'PPI::Future::Element') ? shift : $self->{root};
	my $indent  = shift || '';
	my $output  = shift || [];

	# Print the element if needed
	my $show = 1;
	if ( $Element->isa('PPI::Future::Token::Whitespace') ) {
		$show = 0 unless $self->{display}->{whitespace};
	} elsif ( $Element->isa('PPI::Future::Token::Comment') ) {
		$show = 0 unless $self->{display}->{comments};
	}
	push @$output, $self->_element_string( $Element, $indent ) if $show;

	# Recurse into our children
	if ( $Element->isa('PPI::Future::Node') ) {
...
sub _element_string {
	...
	if ( $self->{display}->{locations} ) {
		my $loc_string;
		if ( $Element->isa('PPI::Future::Token') ) {
	...
	} elsif ( $Element->isa('PPI::Future::Structure') ) {
		# Add the content
		if ( $self->{display}->{content} ) {
