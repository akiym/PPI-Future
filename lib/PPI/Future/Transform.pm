package PPI::Future::Transform;

=pod

=head1 NAME

PPI::Future::Transform - Abstract base class for document transformation classes

=head1 DESCRIPTION

C<PPI::Future::Transform> provides an API for the creation of classes and objects
that modify or transform PPI::Future documents.
...
sub register_apply_handler {
	my $class   = shift;
	my $handler = _CLASS(shift) or Carp::croak("Invalid PPI::Future::Transform->register_apply_handler param");
	my $get     = _CODE(shift)  or Carp::croak("Invalid PPI::Future::Transform->register_apply_handler param");
	my $set     = _CODE(shift)  or Carp::croak("Invalid PPI::Future::Transform->register_apply_handler param");
	if ( $HANDLER{$handler} ) {
		Carp::croak("PPI::Future::Transform->apply handler '$handler' already exists");
	}
...
__PACKAGE__->register_apply_handler( 'PPI::Future::Document', sub { $_[0] }, sub() { 1 } );
...
=head2 new
  ...
  my $transform = PPI::Future::Transform->new(
      param1 => 'value1',
      param2 => 'value2',
  );

The C<new> constructor creates a new object for your C<PPI::Future::Transform>
...
Returns a new C<PPI::Future::Transform>-compatible object, or returns
C<undef> on error.
...
takes a single argument of a L<PPI::Future::Document> object, modifying it
...
that wasn't a L<PPI::Future::Document>, but may include additional errors
...
argument must be a L<PPI::Future::Document>, or something which can be turned
...
sub apply {
	...
	# Get, change, set
	my $Document = _INSTANCE($handler->[0]->($it), 'PPI::Future::Document')
		or Carp::croak("->apply handler for $class failed to get a PPI::Future::Document");
...
sub file {
	...
	# Process the file
	my $Document = PPI::Future::Document->new( "$input" ) or return undef;
	$self->document( $Document )                  or return undef;
